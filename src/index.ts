...
import { DevToolsLocal, DevToolsTunnel } from './lib/RemoteDevTools'
...
export class Plugin extends PuppeteerExtraPlugin {
  ...
  get defaults() {
    return {
      ...
      auth: {
        user: 'user',
        pass: require('node:crypto').randomBytes(20).toString('hex')
      }
    }
  }
   ...
   *
   * @param  {Browser} browser - The browser to create the tunnel for (there can be multiple)
   ...
   * @ignore
   */
  _printGeneratedPasswordWhenNotOverridden(url: string) {
...
export default (pluginConfig?: Record<string, any>) => new Plugin(pluginConfig)
